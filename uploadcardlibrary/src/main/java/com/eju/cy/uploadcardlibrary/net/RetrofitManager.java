...
import com.eju.cy.uploadcardlibrary.BuildConfig;
...
public class RetrofitManager {
    ...
    private OkHttpClient genericClient(final Context context, final String userNo, final String userToken) {
...
//                        Log.w("UploadCard---","userId" + userId + "\n" + token);
    ...
    public AppNetInterface provideClientApi(final Context context, final String userId, final String userToken) {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(SERVER_URL)
                .client(genericClient(context, userId, userToken))
                .addConverterFactory(GsonConverterFactory.create()) //设置使用Gson解析(记得加入依赖)
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create()) // 支持RxJava
                .build();
        return retrofit.create(AppNetInterface.class);
